...
class cpu{
    constructor(program){
        this.program = program; // array of instructions
        this.acc = 0; // accumulator
        this.ip = 0; // instruction pointer
        this.instructions_executed = []; // list of instruction addresses already executed
    }

    parse_inst(addr){
        let split = this.program[this.ip].split(" ");
        return {
            op: split[0],
            val: Number(split[1])
        };
    ...
    tick(){
        this.instructions_executed.push(this.ip);
        let inst = this.parse_inst(this.ip);
        if (inst.op == "nop") this.ip += 1;
        else if (inst.op == "acc"){
            this.acc += inst.val;
            this.ip += 1;
        }
        else if (inst.op == "jmp") this.ip += inst.val;
        else console.error("Invalid operation");
    }

    runUntilLoop(){
        while(!this.instructions_executed.includes(this.ip)){
            this.tick();
        }
        console.log("Exited with accumulator value of", this.acc);
    }

    run(){
        while(true){
            if (this.instructions_executed.includes(this.ip)){
                return {
                    exit_code: 1, //reached a loop
                    acc: this.acc
                };
            }
            else if (this.ip == this.program.length){
                return {
                    exit_code: 0, //exited normally
                    acc: this.acc
                };
            }
            else this.tick();
...
// part 1
let part1 = new cpu(input);
let p1_result = part1.run();
console.log("Part 1: exit code", p1_result.exit_code, "acc value", p1_result.acc);

// part 2 – I'm sure there are more elegant solutions than bruteforcing this but ¯\_(ツ)_/¯
for(let i=0; i<input.length; i++){
    let corrected_program = [...input];
    let instr = corrected_program[i];
    let op = instr.split(" ")[0];
    
    if (op != "acc"){
        if (op == "nop"){
            corrected_program[i] = instr.replace("nop", "jmp");
        }
        else if (op == "jmp"){
            corrected_program[i] = instr.replace("jmp", "nop");
        }
    
        let tmp = new cpu(corrected_program);
        let result = tmp.run();
        if (result.exit_code == 0){
            console.log("Part 2:", tmp.acc);
        }
    }
}